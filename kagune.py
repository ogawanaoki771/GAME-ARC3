...
class WallBreathPocket:
    ...
    def update(self, t, rc):
        ...
        self.pressure = clamp(
            breath * 0.65 + local_rc / RC_MAX * 0.35,
            0.0, 1.0
        )
        self.activity = clamp(
...
class KaguneMatterField:
    ...
    def inject_kagune(self, agent):
        ...
        sources.extend((seg[0], seg[1]) for seg in muscle.segments[::3])
        strength = muscle.energy * 0.72
        for cell in self.cells:
            d = min(distance(cell.x, cell.y, sx, sy) for sx, sy in sources)
            if d > 48:
                continue
            influence = clamp(1.0 - d / 48.0, 0.0, 1.0) * strength
            cell.kagune = max(cell.kagune, influence)
            cell.activity = clamp(cell.activity + influence * MATTER_DIFFUSION, 0, 1)
            cell.fluidity = clamp(cell.fluidity + influence * MATTER_FLOW_GAIN, 0, 1)
            cell.memory = clamp(cell.memory * 0.995 + influence * 0.03, 0, 1)
            if influence > 0.32:
                cell.living = max(cell.living, influence)
                cell.rc = clamp(cell.rc + influence * 0.04, 0, RC_MAX)
    ...
    def statistics(self):
        ...
        return {"bio": float(bio), "fluid": float(fluid), "active": active}
